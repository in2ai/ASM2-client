...
def oauth_dropbox():
    ...
    # Crear el flujo una vez y guardarlo (PKCE)
    if "dbx_auth_flow" not in st.session_state:
        ...
        st.session_state.dbx_code = ""  # búfer del código
    ...
    # Campo persistente (sin formulario)
...
def dropbox_list_files(dbx, root_path):
    ...
    try:
        res = dbx.files_list_folder(
            root, recursive=True, include_non_downloadable_files=False
        )
        while True:
        ...
        st.error(f"Error listando Dropbox en '{root}': {e}")
        st.stop()
    return files
